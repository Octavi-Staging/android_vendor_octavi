...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/octavi/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch octavi_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/octavios/$PROJECT
